...
print("=== RECENT ASTERISK LOG ===")
print(run(c, "docker logs --tail 500 asterisk-beeline 2>&1 | grep -E '3888|8099|sip35|IN Beeline|CID|Dial|INVITE|Hangup|1787734778|77076474703' | tail -80"))
c.close()
...
echo "=== LEADS LAST 5 ===\n";
while ($l = $leads->Fetch()) echo json_encode($l, JSON_UNESCAPED_UNICODE) . "\n";
'''
...
sftp = c2.open_sftp()
with sftp.open("/tmp/_probe_vox_leads.php", "w") as f:
    f.write(probe.encode("utf-8"))
sftp.close()
print(run(c2, "php /tmp/_probe_vox_leads.php 2>&1"))
c2.close()
